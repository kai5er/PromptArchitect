...
const EMPTY_DATA = (): PromptData => ({
  ...
  style: '', color_palette: [], lighting: '', mood: '',
...
export default function App() {
  ...
  useEffect(() => {
    const cards = formPanelRef.current?.querySelectorAll('.reveal-card')
    if (!cards) return
    gsap.to(cards, {
      opacity: 1,
      y: 0,
  ...
  // GSAP hover on randomize button
  const randomizeRef = useRef<HTMLButtonElement>(null)
  const handleRandomizeHover = () => {
    if (!randomizeRef.current) return
    gsap.fromTo(randomizeRef.current, { scale: 1 }, { scale: 1.04, duration: 0.18, ease: 'power2.out' })
  }
  const handleRandomizeLeave = () => {
    if (!randomizeRef.current) return
    gsap.to(randomizeRef.current, { scale: 1, duration: 0.25, ease: 'power2.out' })
  }
  ...
  const addSubject = () => {
    setData(d => ({ ...d, subjects: [...d.subjects, EMPTY_SUBJECT()] }))
  }
  ...
  const handleRandomize = () => {
    gsap.fromTo('.app-body', { opacity: 0.6 }, { opacity: 1, duration: 0.4, ease: 'power2.out' })
    ...
    <div className="app-shell">
      ...
      <header className="app-header">
        <div className="header-brand">
          <div className="header-brand-icon">J</div>
          <h1>Prompt <span>Architect</span></h1>
        ...
        <div className="ops-bar">
          <button
            ...
            onMouseEnter={handleRandomizeHover}
            onMouseLeave={handleRandomizeLeave}
          >
            ⚄ Randomize
          </button>
          <button className="btn btn-ghost" onClick={() => { setImportOpen(true); setImportError('') }}>
            Import
          </button>
          <button className="btn btn-ghost" onClick={handleCopy}>{copyLabel}</button>
          <button className="btn btn-success" onClick={handleDownload}>↓ Export</button>
        </div>
      </header>
      ...
      <div className="app-body">
        ...
        <main className="form-panel" ref={formPanelRef}>
          ...
          <SectionCard icon="◎" title="Global Context">
            <div className="field">
              <label>Scene</label>
              <textarea
                placeholder="Describe the overall scene and setting..."
                value={data.scene}
            ...
            <div className="field-row">
              ...
              <div className="field">
                ...
                <input type="text" placeholder="e.g. rule of thirds..." value={data.composition} onChange={e => setField('composition', e.target.value)} />
              </div>
            </div>
          </SectionCard>

          <SectionCard icon="◈" title="Subjects">
            <div className="subject-list">
              {data.subjects.map((s, i) => (
                <div key={s.id} className="subject-card">
                  <div className="subject-card-header">
                    <div className="subject-label">
                      <div className="subject-index">{i + 1}</div>
                      Subject
                    </div>
                  ...
                  <div className="field-row">
                    <div className="field">
                      <label>Type</label>
                      <input type="text" placeholder="character, creature..." value={s.type} onChange={e => updateSubject(s.id, 'type', e.target.value)} />
                    </div>
                    <div className="field">
                      <label>Position</label>
                      <input type="text" placeholder="foreground center..." value={s.position} onChange={e => updateSubject(s.id, 'position', e.target.value)} />
                    </div>
                  </div>
                  <div className="field">
                  ...
                  <div className="field-row">
                    ...
                    <div className="field">
                      ...
                      <input type="text" placeholder="Facial expression..." value={s.expression} onChange={e => updateSubject(s.id, 'expression', e.target.value)} />
                    </div>
                  </div>
                </div>
              ))}
          ...
          <SectionCard icon="◇" title="Aesthetics">
            <div className="field-row">
              <div className="field">
                <label>Style</label>
                <input type="text" placeholder="e.g. cinematic realism..." value={data.style} onChange={e => setField('style', e.target.value)} />
              </div>
              <div className="field">
                <label>Medium</label>
                <input type="text" placeholder="e.g. digital oil painting..." value={data.medium} onChange={e => setField('medium', e.target.value)} />
              </div>
            </div>
            <div className="field-row">
              <div className="field">
              ...
              <div className="field">
                <label>Mood</label>
                <input type="text" placeholder="e.g. cosmic wonder..." value={data.mood} onChange={e => setField('mood', e.target.value)} />
              </div>
            </div>
            <div className="field">
              <label>Color Palette <span style={{ color: 'var(--text-muted)', fontWeight: 400, fontSize: '11px' }}>— press Enter or comma to add</span></label>
              <ChipInput values={data.color_palette} onChange={v => setField('color_palette', v)} placeholder="midnight indigo, tarnished gold..." />
            </div>
            <div className="field">
              <label>Effects <span style={{ color: 'var(--text-muted)', fontWeight: 400, fontSize: '11px' }}>— press Enter or comma to add</span></label>
              <ChipInput values={data.effects} onChange={v => setField('effects', v)} placeholder="lens flare, film grain..." />
            </div>
          </SectionCard>

          <SectionCard icon="◉" title="Camera Rig">
            <div className="field-row">
              <div className="field">
                <label>Angle</label>
            ...
            <div className="field-row">
              ...
              <div className="field">
                <label>Focus</label>
                <input type="text" placeholder="e.g. bokeh background..." value={data.camera.focus} onChange={e => setCameraField('focus', e.target.value)} />
              </div>
            </div>
          </SectionCard>
      ...
      {importOpen && (
        <div className="modal-backdrop" onClick={e => { if (e.target === e.currentTarget) setImportOpen(false) }}>
          <div className="modal">
            <h2>Import JSON Payload</h2>
            <p>Paste a prompt JSON object below. Subject IDs will be assigned automatically on import.</p>
            <textarea
              value={importText}
